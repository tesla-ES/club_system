...
IF($operation_type){
$sql=" select basic.reg_type,reg_type.Name  , cunt payment_count ,total  from ( SELECT count(*) cunt ,nn.Reg_Type ,sum(total) as total FROM( select User_ID ,employee,Reg_Type ,total  from payment where status in(0,3) and payment.total >=0 and Pay_Year =$payment_year ";
...
$sql .= ") nn group by nn.Reg_Type )basic ,reg_type where basic.reg_type=reg_type.Code  ";
...
<?php report_header();?>
<div class='top_head'>
   إحصائيه باعداد واجمالى الايصالات المسدده
    <?= $payment_year ?><BR>
...
if (($result) || (mysqli_errno($con) == 0)) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($i <= mysqli_num_fields($result)) {
            ...
            $headers[3] = "أعداد الايصالات المسدده";
            $headers[4] = "اجمالى المسدد";
