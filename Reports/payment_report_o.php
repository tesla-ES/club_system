...
<Center><H4> الاعضاء الذين سددوا الاشتراكات عن عام <?=$payment_year?><BR>  نوع العضويه : <?=$Membership_type_name ?>
<hr>
 <?php

  $sql2="SELECT basic_reg.User_ID,basic_reg.Employee,Name,b_date from basic_reg,payment where basic_reg.reg_type=$Membership_type  and  basic_reg.user_id = payment.user_id and basic_reg.reg_type = payment.reg_type and basic_reg.employee= payment.employee and pay_year= $payment_year order by basic_reg.User_ID";
...
if(!($start)) $start=0;
 ...
 $sql="SELECT basic_reg.User_ID,basic_reg.Employee,Name,b_date,case when basic_reg.Reg_Type in(13,15,33,41,42)  then basic_reg.notes  else '' end AS notesو,case when basic_reg.Reg_Type in(13,15,33,41,42)  then payment.exe_val  else '' end AS exe_val from basic_reg,payment where basic_reg.reg_type=$Membership_type  and  basic_reg.user_id = payment.user_id and basic_reg.reg_type = payment.reg_type and basic_reg.employee= payment.employee and pay_year= $payment_year order by basic_reg.User_ID LIMIT $start , $per_page ";
...
if(!($start>=$record_count-$per_page)){
	...
	echo " <a href='payment_report.php?start=$last_rec&Membership_type=$Membership_type&per_page=$per_page&payment_year=$payment_year'><img src='../img/lastpage.gif' alt='Last Page'></a>";
	/////////////getting the next page
echo " <a href='payment_report.php?start=$next&Membership_type=$Membership_type&per_page=$per_page&payment_year=$payment_year'><img src='../img/nextpage.gif' alt='Next Page'></a>";

}
  //show prev button
if(!($start<=0)){
 //////////////////getting previous page
echo " <a href='payment_report.php?start=$prev'&Membership_type=$Membership_type&per_page=$per_page&payment_year=$payment_year'><img src='../img/backpage.gif' alt='Previous Page'></a>";
/////////////getting the first page
echo " <a href='payment_report.php?start=0&Membership_type=$Membership_type&per_page=$per_page&payment_year=$payment_year'><img src='../img/first.gif' alt='First Page'></a>";
}
